...
public class XmppResource {
	...
	// public String hello(){
	// // try {
	// // userDao.updateAppList("af7280ded07443868d0be329ee065216", "blah...");
	...
	public String updateAppList(JSONObject json) throws JSONException {
		log.info("/appList [POST] invoke..");
		String username = json.getString("username");
		String appList = json.getString("applist");

		log.debug("username=" + username);
		log.debug("appList=" + appList);
		if (username != null && appList != null) {
			try {
				userDao.updateAppList(username, appList);
			} catch (UserNotFoundException e) {
				e.printStackTrace();
				return "update fail";
			}
	...
	@Produces(MediaType.TEXT_HTML)
	@Consumes(MediaType.APPLICATION_JSON)
	public String updateUser(JSONObject json) throws JSONException {
		log.info("/user [POST] invoke..");
		String username = json.getString("username");
		String appList = json.getString("applist");
		String adminPolicy = json.getString("adminPolicy");
		String androidID = json.getString("androidID");
		String cpuID = json.getString("cpuID");
		String isRoot = json.getString("isRoot");
		String locationGPS = json.getString("locationGPS");
		String locationWIFI = json.getString("locationWIFI");
		String simInfo = json.getString("simInfo");
		String sysList = json.getString("sysList");
		String telID = json.getString("telID");
...
//			try {
				...
				if (user != null) {
					...
					user.setAdminPolicy(adminPolicy);
					user.setAndroidID(androidID);
					user.setCpuID(cpuID);
					user.setIsRoot(isRoot);
					user.setLocationGPS(locationGPS);
					user.setLocationWIFI(locationWIFI);
					user.setSimInfo(simInfo);
					user.setSysList(sysList);
					user.setTelID(telID);
...
//			}
	...
	public String getUserInfo(@PathParam("username") String username) {
		log.info("/info/" + username + " [GET] invoke..");
//		try {
			User user = userDao.getUserByUsername(username);
			if( user == null ){
				return "Get user failed"; 
			}else{
				return user.getAppList();
			}
//		} catch (UserNotFoundException e) {
//			e.printStackTrace();
//		}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	public JSONObject push(JSONObject param) {
		log.debug("/xmpp/push..");

		JSONObject responseJson = new JSONObject();
		try {
			responseJson.put("status", "error");
			responseJson.put("msg", "wrong input");
		...
		if (pushResult == null) {
			try {
				responseJson.put("status", "Error");
				responseJson.put("msg", "Thread pool is full");
		...
		try {
			responseJson.put("id", uuid);
			responseJson.put("status", "ok");
			responseJson.put("msg", "finished");
		} catch (JSONException e) {
			e.printStackTrace();
		}
		...
		try {
			...
			if (type.toLowerCase().compareTo("confirm") == 0) {
				if (data.toLowerCase().compareTo("ok") == 0) {
					isOk = PushStatusMap.INSTANCE.changeStatus(id, PushStatus.FinishedOK);
				} else if (data.toLowerCase().compareTo("error") == 0) {
					isOk = PushStatusMap.INSTANCE.changeStatus(id, PushStatus.FinishedError);
				}
			}
			if (!isOk) {
				resJson.put("status", "Error");
				resJson.put("msg", "No such ID");
				return resJson;
			}
	...
	public JSONObject register(JSONObject param) {
		...
		if( u != null ){
//			log.info( u.toString() + " do exists!!" );
			try {
				responseJson.put("status", "duplicate");
				responseJson.put("id", u.getId());
				return responseJson;
			} catch (JSONException e) {
				e.printStackTrace();
			}
...
//		
		try {
			...
			e1.printStackTrace();
			try {
				responseJson.put("msg", "user insert error:" + e1.getMessage());
			} catch (JSONException e) {
				e.printStackTrace();
			}
			return responseJson;
		...
		try {
			responseJson.put("status", "ok");
			responseJson.put("msg", "user created");
	...
	public JSONObject confirm(JSONObject param) {
		...
		log.info("Input JSON:" + param.toString());
		System.out.println("Input JSON:" + param.toString());
		...
		try {
			responseJson.put("status", "error");
			responseJson.put("msg", "no such user");
			responseJson.put("id", "-1");
		} catch (JSONException e) {
			e.printStackTrace();
		}
...
//		}
		...
		try {
			responseJson.put("status", "error");
			responseJson.put("msg", "wrong password");
			responseJson.put("id", "-1");
		} catch (JSONException e) {
			e.printStackTrace();
		}
		...
		try {
			responseJson.put("status", "ok");
			responseJson.put("msg", "user comfirm ok");
			responseJson.put("id", user.getId());
		} catch (JSONException e) {
			e.printStackTrace();
		}
	...
	@Produces(MediaType.APPLICATION_JSON)
	public JSONObject deleteUserById(@PathParam("userid") String id) {
	...
	@Consumes(MediaType.TEXT_HTML)
	@Produces(MediaType.APPLICATION_JSON)
	public JSONObject getUser(@PathParam("userid") String id) {
		log.info("/user/" + id + " [DELETE] invoke..");
		...
		try {
			...
			responseJson.put("presence", presence);
		} catch (JSONException e) {
			e.printStackTrace();
		}
	...
	public String pushStatus(@QueryParam("id") String id) {
