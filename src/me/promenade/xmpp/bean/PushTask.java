...
public class PushTask implements Runnable, Serializable {
	...
	private String pushMsg(PushMessage msg){
        ...
        for (ClientSession session : sessionManager.getSessions()) {
        	...
        	System.out.println( "session address:" + session.getAddress().toString() );
        	System.out.println( "session address JID:" + session.getAddress().toFullJID().toString() );
	...
	public void run() {
		Log.info("starting thread@" + threadIndex + " uuid:" + threadUUID);
		this.status = PushStatus.Sending;
		String xml = pushMsg(msg);
		Log.info("thread@" + threadIndex + "--->" + xml);
		Log.info("End .. thread@" + threadIndex);
